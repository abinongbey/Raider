import os
import shutil

try:
    import pyzipper
    import colorama
except ImportError:
    _ = os.system('pip install colorama' if os.name == 'nt' else 'pip3 install colorama')
    _ = os.system('pip install pyzipper' if os.name == 'nt' else 'pip3 install pyzipper')
import pyzipper
import sys
from time import sleep
from getpass import getpass
from colorama import Fore

# -------------------Colors
r = "\033[1;31m"
g = "\033[1;32m"
y = "\033[1;33m"
b = "\033[1;34m"
d = "\033[2;37m"
R = "\033[1;41m"
Y = "\033[1;43m"
B = "\033[1;44m"
w = "\033[1;37m"
g = "\033[0;90m"
y = r

# ------------------banners
logo = Fore.GREEN + """
  _       _____                  _     __
| |     / /   |     _________ _(_)___/ /__  _____
| | /| / / /| |    / ___/ __ `/ / __  / _ \/ ___/
| |/ |/ / ___ |   / /  / /_/ / / /_/ /  __/ /
|__/|__/_/  |_|  /_/   \__,_/_/\__,_/\___/_/                         

                                 By Abinong Bey 

"""


def banner():
    print(logo)

def cls_clear_func():
    os.system('cls' if os.name=='nt' else 'clear')

def exixting_directory_file(file):
    if os.path.exists(file):
        os.remove(file)

def designprint(abi):
    print(r+"└─> "+w+"\033[1;37m"+abi)

def front_design():
    cls_clear_func()
    banner()

front_design()


class Setup:
    def __init__(self,user):
        self.data = user
    def mainFile(self):
        self.save = self.data
        try:
            with pyzipper.AESZipFile('main.zip', 'r', compression=pyzipper.ZIP_DEFLATED,
                                     encryption=pyzipper.WZ_AES) as extracted_zip:
                extracted_zip.extractall(pwd=str.encode(self.save))
            designprint('Password Correct !')
            sleep(1)
            front_design()
            designprint('Successfully unzipped')
            sleep(1)
            exixting_directory_file('main.zip')
            os.system('python main.py' if os.name=='nt' else 'python3 main.py')
        except Exception as samay:
            designprint('Password Incorrect !')
            os.system('python Run.py' if os.name=='nt' else 'python3 Run.py')


try:
    user_ezip_unzipping = getpass(r+"└─"+w+"\033[1;37mZipfile pass : "+r).strip()
except:
    pass

if __name__ == '__main__':
    exixting_directory_file('main.py')
    main_start = Setup(user_ezip_unzipping)
    main_start.mainFile()





